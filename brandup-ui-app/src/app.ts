...
export class Application<TModel extends AppClientModel> extends UIElement implements IApplication {
    ...
    protected constructor(model: TModel, options: AppSetupOptions) {
        ...
        this.model = model;
        this.options = options ? options : { defaultPageScript: null };
    ...
    private __loadPageScript(navState: PageNavState, pageModel: PageClientModel, contentHtml: string, scrollToTop: boolean, pushState: boolean) {
        ...
        if (pageScript) {
            this.__builder.getPageType(pageScript)
                .then((pageType) => {
                    this.__createPage(pageType.default, navState, pageModel, contentHtml, scrollToTop, pushState);
                })
                .catch(() => {
            ...
            this.__createPage(Page, navState, pageModel, contentHtml, scrollToTop, pushState);
    }
    private __createPage(pageType: new (...p) => Page<PageClientModel>, navState: PageNavState, pageModel: PageClientModel, contentHtml: string, scrollToTop: boolean, pushState: boolean) {
        if (this.page) {
            this.page.destroy();
            this.page = null;
        }
